class Paginator {
    ...
    paginate = async (pages) => {
        ...
        const reactionCollector = curPage.createReactionCollector((reaction, user) => [reaction.emoji.id, reaction.emoji.name].some(react => this.emojiList.includes(react)) && !user.bot && user.id === this.msg.author.id, { time: this.timeout });
